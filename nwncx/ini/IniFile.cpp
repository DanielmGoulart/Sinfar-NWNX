...
FILE* operator<<(FILE* output, CIniFileA& obj)
{
    obj.Save( output );
    return output;
}

FILE* operator>>(FILE* input, CIniFileA& obj)
{
    obj.Load( input );
    return input;
}

FILE* operator>>(FILE* input, CIniMergeA merger)
{
    return merger(input);
}
...
void CIniFileA::Save( FILE* output )
{
    std::string sSection;
...
void CIniFileA::Load( FILE* input , bool bMerge )
{
    if( !bMerge )
        RemoveAllSections();
...
std::string CIniSectionA::GetKeyValue(const std::string& sKey, const std::string& sDefaultValue) const
{
#ifdef _CINIFILE_DEBUG
