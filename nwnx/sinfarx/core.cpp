...
namespace core
{
	...
	int module_loaded_hook(CNWSModule* module)
	{
		...
		for (auto& hook: get_hooks(hook::module_loading))
		{
			hook();
		}
		...
		for (auto& hook: get_hooks(hook::module_loaded))
		{
			hook();
		}
	...
	{
		mysql_admin.reset(new CMySQLCon("localhost", "xxx", "xxx", "xxx"));
		current_server_id = mysql_admin->query_int("SELECT using_id FROM servers WHERE port=" + std::to_string(current_server_port));
		mysql_sinfar.reset(new CMySQLCon("localhost", "xxx", "xxx", "xxx"));
		char mysql_restricted_user[100];
		char mysql_restricted_db[100];
		int server_database_id = current_server_id & sinfar_vault_id ? 1 : current_server_id;
		int server_db_port = current_server_id & sinfar_vault_id ? 5121 : current_server_port;
		sprintf(mysql_restricted_user, "sfserv_%d", server_database_id);
		sprintf(mysql_restricted_db, "sfserv_%d", server_database_id);
		std::vector<std::string> db_password = mysql_admin->query_row("SELECT dbpassword,pending_dbpassword FROM servers WHERE port=" + std::to_string(server_db_port));
		mysql_restricted.reset(new CMySQLCon("localhost", mysql_restricted_user, db_password[0]==""?"xxx":db_password[0], mysql_restricted_db));
		if (db_password[1] != "" && mysql_restricted->set_password(db_password[1]))
		{
			mysql_admin->query("UPDATE servers SET pending_dbpassword=NULL, dbpassword=" + mysql_admin->quote(db_password[1]) + " WHERE port=" + std::to_string(server_db_port));
		}
		
		const char* init_db_queries[] = {
			"SET SESSION wait_timeout=(60*60*24)",
			"SET sql_mode='STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_AUTO_CREATE_USER,NO_ENGINE_SUBSTITUTION'"
		};
		for (uint32_t query_index=0; query_index<sizeof(init_db_queries)/sizeof(char*); query_index++)
		{
			mysql_admin->query(init_db_queries[query_index]);
			mysql_sinfar->query(init_db_queries[query_index]);
			mysql_restricted->query(init_db_queries[query_index]);
		}

		mysql_admin->query("UPDATE pcs_status"
			" LEFT JOIN pcs ON(pcs.pc_id=pcs_status.pc_id)"
			" LEFT JOIN players ON(players.player_id=pcs.player_id)"
			" SET leave_date=CURRENT_TIMESTAMP"
			" WHERE leave_date IS NULL AND players.current_server=" + std::to_string(current_server_port));

		mysql_admin->query("UPDATE players SET current_server=NULL WHERE current_server=" + std::to_string(current_server_port));
	}
	...
	{
		current_server_port = 5121;
		int i;
		for (i=0; i<argc; i++)
		{
			if (strcmp(argv[i], "-port")==0)
			{
				current_server_port = atoi(argv[i+1]);
				break;
			}
		}
		if (i==argc)
		{
			fprintf(stderr, "NWNXMYFUNCS: -port not found, using 5121\n");
		}

		prepare_databases();

		for (auto& hook: get_hooks(hook::init))
		{
			hook();
		}

		//allow debugging
		/*if (current_server_port == 5130)
		{
			fprintf(stderr, "attach to process...\n");
			char wait_debugger;
